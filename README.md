...
#### (1) User Function: Search by User
...
### About Tries, also called Prefix Trees
A trie is a tree made up of nodes, each of which represents a string character and points to as few as zero child nodes and may point to more than two child nodes. Each node has a vector pre-initialized with the number of possible characters that could be used (ex: 26 for each letter of the English alphabet). To indicate the character "C", for example, the address of the node would occupy index 2 of the node’s predecessor node’s descendants vector. This differs from binary trees whose nodes point to, at most, two child nodes.
#### Traversal Method
...
To traverse the tree, we do not need to check each node for a string character. Instead, we use the traversal path to track the string we store. Each TrieNode stores a vector called descendants of TrieNode* pointers. The vector is pre-initialized with 37 slots, each of which points to NULL. Each index is associated with one for the 37 possible characters we could read in the user input: A-Z are associated with indices 0-25, 26 is the dash character, and indices 27-36 are associated with the string characters 0-9. 
So, to traverse the tree based on a string, we will calculate the index of each character by doing index arithmetic. We can use the ASCII value of the current character and subtract it from a suitable ‘base’ to provide the correct offset in the descendants vector. If the current character is a “C”, we subtract “C” and  “A”, giving us index 2. So we access the current node's descendants vector at index 2. To handle 0-9, we subtract the string “4” from string “0”, then add 26 to skip past “A-Z”. This indexing approach helps achieve faster traversal because we don’t need to iterate through each node’s descendant’s vector at every level of the trie. If we calculate an index and there’s a nullptr, for search, the function returns null.
#### Search
When searching, the function returns true if a course subject code is in the Trie and false when it is not.
#### AutoComplete, or Prefix Search
This works very similarly to search. If there is a mismatch in characters provided by the user (for example, they input "CSPB" but all strings only start with the prefix "CSCI", then it returns false. 
However, tries also can output the string for every matching instance. If the user input "CSCI", the function will list every course subject code that begins with "CSCI" contained in the trie.  
#### Insertion
When a new course is added to the trie, new nodes are only created for the course's string characters that differ from the existing courses. We use the same traversal method as noted above. The first part of the function is similar to search. However, instead of returning false when there is nullptr at the desired index, we create a new node, update the pointer at the appropriate index to point to it, navigate to it, and keep creating new nodes for each remaining character in the current string. When we reach the leaf node, we point the Course pointer variable in the 
#### Removal 
...
### Why a Trie?
#### Usage Alignment
First, tries are designed to work with strings, and since all the course information is string-based, there is natural alignment. 
...
#### Time Complexity
Search, insertion, autocomplete, and removal have a complexity of O(N), where N is the length of the subject code. My implementation features very short actual run time because the course subject codes used are only 9 characters in length. Additionally, insertion only happens when the trie is built and there is not a use case for us to remove just one course from the trie. Trie deconstruction is the only time nodes are removed and deleted. The only two functions used repeatedly by the user are Search and AutoComplete which are efficient.
However, as noted below, the space complexity is not good for deletion and overall from an auxiliary space perspective. There is wasted storage space for each node in the trie because there are a maximum 37 nullptrs in each node and not every node will use all of the pointers. 
...
## File Structure
Here is the file structure of the repository and the internal data members for the 3 primary custom Classes I made, Trie, TrieNode, and Course. 
### /code directory
#### Course Data
...
These support a custom class Trie. Each trie stores the number of courses in the trie and a pointer to the root of the tree. 
  ...
  public:
    ...
    bool getUserStatus();
    string consoleOutputWordWrapping(string course_subject_code, const int WIDTH_OF_LINE); ////this one returns a string that has been prepared for Word wrapping
   ```
##### Course.h & Course.cpp
...
However, once I integrated the new web UI, the file structure could not accommodate this and therefore neither feature works as it did previously.
#### ./app directory
...
##### /app/main.cpp
This file originally ran the 'Run Apps' button by reading the data from the .csv file (see below), building the trie, and handling input/output through the terminal via CLI. 
...
##### /test/test_proj.cpp
This file contains my Unit tests for my 3 custom classes: Trie, TrieNode, and Course related functions, including setter/getters, constructor and deconstructors, key functions like buildTrie, readData, and more.
This is no longer an actively supported feature and I was unable to make the necessary changes in time. This does not affect the functionality of the program or the web UI.
...
#### Design issue - Unnecessary deletion in removeNode()
When I was writing the function to delete nodes, I made a bad design decision. I had the removeNode() function delete all nodes related to a course when removing it from the trie. After deleting a leaf node for the doom course, I would travel up to the doom node's predecessor and if there was only one descendant (the leaf node), I would delete that internal node as well. The documentation I was referring to (see references) suggested that it was also acceptable to simply mark the leaf node as a non-leaf, reassign the node's Course* pointer to nullptr, and stop. I took the advice and did so. As noted previously, I decided to rely on the deconstructor to handle node deletion.
...
#### Network Issues
Thanks to Prof. Guinn, he resolved a network issue that resolved a lookup error message anytime I tried to run searches using the html page: "Request failed: TypeError: NetworkError when attempting to fetch resource." This required me to use a different locally hosted URL for the web UI since the program uses JupyterHub as a proxy server.
